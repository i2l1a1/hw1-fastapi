...
class TestDB(unittest.TestCase):
    def test_get_db_returns_session_and_closes_it(self):
        ...
        )  # сохраняем ссылку на реальную сессию
        db_module.SessionLocal = (
            fake_session_local  # подменяем SessionLocal на фейковую сессию
        )
        ...
        try:
            ...
            self.assertIs(
                session_from_generator, fake_session
            )  # убеждаемся, что пришла именно фейковая сессия

            with self.assertRaises(StopIteration):
                next(
                    generator
                )  # второй next() нужен, чтобы генератор завершился и вызвал close()
        finally:
            db_module.SessionLocal = (
                original_session_local  # возвращаем настоящую сессию
            )
