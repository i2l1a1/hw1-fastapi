...
> Покрытие тестами выводится в консоль, а также сохраняется в `coverage.xml`. Сейчас покрытие тестами составляет 98%. Чтобы отдельно посмотреть на покрытие, можно ввести команду `coverage report` (заскринил это на `coverage_report_results.png`).
...
> Я создал `pyproject.toml` и `.pre-commit-config.yaml` для конфигурации (проще всего посмотреть эти файлы, там всё понятно, а словами описывать их долго).

1. Один раз нужно выполнить `pre-commit install`.
2. Перед коммитом `pre-commit` запустит `ruff`, `black` и `isort` (заскринил это на `pre_commit_results_real_commit.png`). При необходимости можно запустить
   вручную: `pre-commit run --all-files` (заскринил это на `pre_commit_results.png`).
