...
> Покрытие тестами выводится в консоль, а также сохраняется в `coverage.xml`. Сейчас покрытие тестами составляет 98%. Чтобы отдельно посмотреть на покрытие, можно ввести команду `coverage report` (заскринил это в `coverage_report_results.png`).
...
2. Перед коммитом `pre-commit` запустит `ruff`, `black` и `isort`. При необходимости можно запустить
   вручную: `pre-commit run --all-files` (заскринил это в `pre_commit_result.png`).
